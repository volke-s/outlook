...
export class page extends popup {
    ...
    retrieve() {
        ...
        switch (provider_id) {
            case "outlook":
                ...
                const name = this.get_element('user_name').value;
                //
    ...
    async check() {
        ...
        const name_is_valid = is_valid('user_name');
        //
...
class outlook extends provider {
    ...
    name;
    password;
    constructor(name, password, operation) {
    ...
    async authenticate() {
        ...
        if (this.operation_id === "register") {
            ...
            await server.exec("database", ["mutall_users"], "register", [this.name, this.password]);
        }
        else {
            //
            //LOGIN
            //Authenticate the user using the given name and password
            const ok = await server.exec("database", ["mutall_users"], "authenticate", [this.name, this.password]);
            //
